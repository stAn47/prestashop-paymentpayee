 ...
 * 
 * Modified by stAn to support payee plugin method
 * tested on: https://{domain}/en/module/paymentpayee/wkcron?token={wk cron token}
 */
if (!defined('_PS_VERSION_')) {
    exit;
}
//PaymentPayeeValidationModuleFrontController
//is a clone of WkProductSubscriptionCronModuleFrontController
class PaymentPayeeWkcronModuleFrontController extends ModuleFrontController
{
    public $todayDate;
    ...
    public function init()
    {
        ...
        if (Module::isEnabled('wkproductsubscription')) {
            include_once _PS_MODULE_DIR_ . 'wkproductsubscription/classes/WkSubscriptionRequired.php';
             ...
             exit('Requires wkproductsubscription');
        }
        // Validate token
        ...
        if (isset($token)
            ...
            if ($tomScheduledOrders) {
                foreach ($tomScheduledOrders as $subsData) {
                    if ($subsData['payment_module'] == 'paymentpayee' && Module::isEnabled('paymentpayee')) {
                        $payeeResponse = json_decode($subsData['payment_response'], true);
                        if (!$payeeResponse) {
                            continue; // becasue not found token
                        }
                    }
            ...
            if ($todayScheduledOrders) {
                foreach ($todayScheduledOrders as $subscriptionData) {
                    if ($subscriptionData['payment_module'] == 'paymentpayee' && Module::isEnabled('paymentpayee')) {
                        $payeeResponse = json_decode($subscriptionData['payment_response'], true);
                        if ($payeeResponse) {
                            //Need to check subscription is active or not
                    ...
                    if ($idCart) {
                                ...
                                foreach ($cartRules as $cartRule) {
                                    ...
                                    if (Validate::isLoadedObject($objCartRule)) {
                                        ...
                                        if ($objCartRule->free_shipping) {
                                            $objCartRuleNew->free_shipping = 1;
                                        }
                        ...
                        if ($subscriptionData['payment_module'] == 'paymentpayee' && Module::isEnabled('paymentpayee')) {
                            ...
                            if ($payeeResponse) {
                                //Need to check subscription is active or not
                                $idOrder = $this->createSubscriptionOrderForPayee($subscriptionData,
                                (int) $idCart);
                            } else {
                                continue; // becasue not found token
                            }
     ...
     * @return void
     */
    public function createSubscriberCart($subscriptionData)
    {
    ...
    private function createSubscriptionOrderForPayee($subscriptionData, $id_cart)
    {
     ...
     */
    public function createSubscriptionOrder($subscriptionData, $id_cart)
    {
        $module_name = $subscriptionData['payment_module'];
